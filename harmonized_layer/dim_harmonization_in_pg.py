...
spark = SparkSession.builder.appName("Harmonization").getOrCreate()

# Fucntion to check if the harmonized table exists
def check_table_exists(harmonized_table):
    catalog, schema, table = harmonized_table.split('.')
    check_sql = f"""
            select
                true
            from
                processing_catalog.information_schema.tables
            where
...
def data_quality_checks(df, primary_key, audit_column, check_columns):
    # duplicate checks
...
# Main ETL Function
def process_harmonized_table(RAW_TABLE,HARMONIZED_TABLE,DQ_TABLE,PRIMARY_KEY,AUDIT_COLUMN):
    ...
    dq_failures = data_quality_checks(latest_batch_df, PRIMARY_KEY, AUDIT_COLUMN)
...
if __name__ == "__main__()":
    ...
    # audit_column = 'r_insert_timestamp'
    # check_columns = ["product_name", "brand", "category","gender", "size", "color", "price", "availability"]

    parser = argparse.ArgumentParser(description="Harmonize dimension tables")
    parser.add_argument("--raw_table", required=True, help="Path to raw table")
    parser.add_argument("--harmonized_table", required=True, help="Path to harmonized table")
    parser.add_argument("--dq_table", required=True, help="Path to DQ log table")
    parser.add_argument("--primary_key", required=True, help="Primary key column name")
    parser.add_argument("--audit_column", required=True, help="Audit column name")
    parser.add_argument("--check_columns", required=True, help="Comma-separated list of columns to check for nulls")

    args = parser.parse_args()

    # Convert check_columns string to list
    check_columns = args.check_columns.split(',')

    process_harmonized_table(
        args.raw_table,
        args.harmonized_table,
        args.dq_table,
        args.primary_key,
        args.audit_column,
        check_columns
...
#### Usage ####
# if using spark-submit(not need to do this in Databricks)
